...
static void ZstdAsyncDecompressExecute(napi_env env, void* data) {
    ...
    // ZSTD_decompress 是一键式 API：自动解析帧头并解压所有数据
...
static void Lz4AsyncCompressExecute(napi_env env, void* data) {
    ...
    // 1. 创建压缩上下文，LZ4 流式 API 需要上下文来保持内部状态
    LZ4F_cctx* cctx = nullptr;
    LZ4F_errorCode_t err = LZ4F_createCompressionContext(&cctx, LZ4F_VERSION);
    if (LZ4F_isError(err)) {
        async_data->success = false;
        async_data->error_message = LZ4F_getErrorName(err);
        return;
    }

    // 2. 配置压缩参数
    LZ4F_preferences_t prefs = LZ4F_INIT_PREFERENCES;
    if (async_data->compression_level > 0) {
        prefs.compressionLevel = async_data->compression_level;
    }

    // 3. 写入帧头（魔数 0x184D2204 + 元数据），返回帧头大小
    size_t dst_offset = 0;
    ...
    if (LZ4F_isError(header_size)) {
        ...
        async_data->success = false;
        async_data->error_message = LZ4F_getErrorName(header_size);
        return;
    }
    dst_offset += header_size;

    // 4. 压缩数据块，这是"流式"的核心步骤
    size_t remaining = async_data->output_capacity - dst_offset;
    size_t block_size = LZ4F_compressUpdate(
        cctx,
        static_cast<char*>(async_data->output_data) + dst_offset, remaining,
        async_data->input_data, async_data->input_size,
        nullptr);
    if (LZ4F_isError(block_size)) {
        LZ4F_freeCompressionContext(cctx);
        async_data->success = false;
        async_data->error_message = LZ4F_getErrorName(block_size);
        return;
    }
    dst_offset += block_size;

    // 5. 写入帧尾（结束标记 + 校验和），结束帧
    remaining = async_data->output_capacity - dst_offset;
    ...
    if (LZ4F_isError(end_size)) {
        LZ4F_freeCompressionContext(cctx);
        async_data->success = false;
        async_data->error_message = LZ4F_getErrorName(end_size);
        return;
    }
    dst_offset += end_size;
    ...
    async_data->success = true;
    async_data->compressed_size = dst_offset;
}
...
static void Lz4AsyncDecompressExecute(napi_env env, void* data) {
    ...
    // 2. 读取帧头，获取帧信息（包括原始数据大小 contentSize）
    LZ4F_frameInfo_t frame_info = LZ4F_INIT_FRAMEINFO;
    size_t src_pos = async_data->input_size;
    size_t hint = LZ4F_getFrameInfo(dctx, &frame_info,
                                    async_data->input_data, &src_pos);
    if (LZ4F_isError(hint)) {
        LZ4F_freeDecompressionContext(dctx);
        async_data->success = false;
        async_data->error_message = LZ4F_getErrorName(hint);
        return;
    }

    // 3. 根据帧头信息分配输出缓冲区
    //    contentSize 存储了原始数据大小（如果 LZ4 帧中记录了的话）
    size_t output_capacity = 0;
    if (frame_info.contentSize > 0) {
        output_capacity = static_cast<size_t>(frame_info.contentSize);
    } else {
        // 保守估计：LZ4 最大块大小为 4MB，缓冲区不足时自动扩容
        output_capacity = async_data->input_size * 10;
    }

    // 第一次分配输出缓冲区
    size_t alloc_size = output_capacity;
    char* output_buffer = new char[alloc_size];

    // 4. 循环解压每个数据块，hint == 0 表示所有块解压完毕
    size_t src_offset = src_pos;
    size_t dst_offset = 0;

    while (hint != 0) {
        size_t src_size = async_data->input_size - src_offset;
        size_t dst_size = alloc_size - dst_offset;

        // 输出缓冲区不够时自动扩容（最多尝试 10 次）
        if (dst_size == 0) {
            ...
            if (new_size <= alloc_size) {
                ...
                LZ4F_freeDecompressionContext(dctx);
                async_data->success = false;
                async_data->error_message = "Output buffer overflow during LZ4 decompression";
                return;
            }
            char* new_buffer = new char[new_size];
            std::memcpy(new_buffer, output_buffer, dst_offset);
            delete[] output_buffer;
            output_buffer = new_buffer;
            alloc_size = new_size;
            dst_size = alloc_size - dst_offset;
        }

        hint = LZ4F_decompress(
            dctx,
            output_buffer + dst_offset, &dst_size,
            static_cast<const char*>(async_data->input_data) + src_offset, &src_size,
            nullptr);
    ...
    // 6. 保存解压结果
    async_data->output_data = output_buffer;
    async_data->output_size = dst_offset;
    async_data->success = true;
}
...
static void SnappyAsyncCompressExecute(napi_env env, void* data) {
    ...
    size_t compressed_size = async_data->output_capacity;
    snappy_status status = snappy_compress(
        static_cast<const char*>(async_data->input_data),
        async_data->input_size,
        static_cast<char*>(async_data->output_data),
        &compressed_size);

    if (status != SNAPPY_OK) {
        async_data->success = false;
        async_data->error_message = "Snappy compression failed";
    } else {
        async_data->success = true;
        async_data->compressed_size = compressed_size;
    }
}
...
static void SnappyAsyncDecompressExecute(napi_env env, void* data) {
    ...
    size_t uncompressed_size = async_data->output_size;
    ...
    if (status != SNAPPY_OK) {
        async_data->success = false;
        async_data->error_message = "Snappy decompression failed";
    } else {
        async_data->success = true;
    }
}
...
// 获取版本信息：返回 "MemoryDecompressor v1.0.0 (ZSTD x.y.z, LZ4 x.y.z)"
